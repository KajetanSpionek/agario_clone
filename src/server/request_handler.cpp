...
namespace websocket {
    namespace http {
        ...
        void RequestHandler::handleRequest(const Request& req, Reply& rep)
        {
            ...
            std::string accept(toBase64(toSha1(buffer)));
        ...
        std::vector<unsigned char> RequestHandler::toSha1(const std::string& s)
        {
        ...
        std::string RequestHandler::toBase64(const std::vector<unsigned char>& data)
        {
